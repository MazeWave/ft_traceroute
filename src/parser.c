...
int parse_args(int argc, char **argv, t_tr *tr)
{
	...
	while ((opt = getopt(argc, argv, "-h?m:q:w:f:p:")) != -1)
	{
		...
		case 'q':
			...
			if (atof(optarg) < 0)
			return (
			printf(RED "Error: At least one proble per hop is requiered\n" RESET),
			help(tr), tr->exit_status = true);
			break;
		case 'w':
			if (!tr->is_bonus)
			...
			return (
			printf(RED
			    "Error: Response time must be at 1 least seconds\n" RESET),
			help(tr), tr->exit_status = true);
		...
		case 'l':
			...
			return (
			printf(RED "Error: The max hop must be between 0 and 64\n" RESET),
			help(tr), tr->exit_status = true);
			tr->max_hops = atoi(optarg);
		...
		case 'p':
			...
			if (atoi(optarg) <= 0 || atoi(optarg) > 65535)
			return (
			printf(RED "Error: The port must be between 0 and 65535\n" RESET),
			help(tr), tr->exit_status = true);
			tr->port = atoi(optarg);
			LOG(BLUE "port: %d" RESET, tr->port);
			break;
